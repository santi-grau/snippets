...
class Ambient extends Object3D{
    constructor( positions ){
        ...
        this.g = new Geo( this.positions, 1024, this.positions.newDataStream( false, 1024 ).id, [ 'PlaneTextMaterial', 'PlaneTextMaterial' ], geometryOptions )
        this.add( this.g )

        this.g2 = new Geo( this.positions, 1024, this.positions.newDataStream( false, 1024 ).id, [ 'PlaneTextMaterial', 'PlaneTextMaterial' ], geometryOptions )
        // this.add( this.g2 )
    }

    simulatePosition( time, seed ){
        var x = this.simplex.noise3D( 130 + seed , 100, time * 0.00025 ) * 2
        var y = this.simplex.noise3D( 140, time * 0.00025, 12 + seed ) * 1.75 + 0.25
        var z = this.simplex.noise3D( time * 0.00025, 1, 10) * 2 + 0.1
