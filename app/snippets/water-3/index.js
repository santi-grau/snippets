import { Vector2, Raycaster, PerspectiveCamera, Scene, Points, WebGLRenderer,BufferGeometry, LineBasicMaterial, LineSegments, BufferAttribute, PlaneBufferGeometry, Mesh, MeshBasicMaterial, Vector3, BoxBufferGeometry, ShaderMaterial, OrthographicCamera } from 'three'
import WaterMesh from './WaterMesh'
import geoShader from './geo.*'
import gridShader from './grid.*'
import dotShader from './points.*'
...
class Grid extends LineSegments{
    constructor( size ){
        ...
        // for( var i = 0 ; i < this.res; i++ ) {
        //     vs.push(  i / ( this.res-1 ) * this.size - this.size / 2, 1000, this.size - this.size / 2 )
        //     uvs.push( i / ( this.res - 1 ), 1 )
        //     indices.push( ( this.res - 1 ) + this.res * i, this.res * this.res + i )
        // }

        // for( var i = 0 ; i < this.res; i++ ) {
        //     vs.push(  this.size - this.size / 2, 1000, i / ( this.res-1 ) * this.size - this.size / 2 )
        //     uvs.push( 0, 0 )
...
class Geo extends LineSegments{
    constructor( size ){
        ...
        for( var i = 0 ; i < this.depth ; i++ ){
            for( var j = 0 ; j < this.res ; j++ ){
                vs.push( j / ( this.res - 1 ) * this.size - this.size / 2, -(i + 1) * this.step, this.size - this.size / 2 )
                uvs.push( j / ( this.res - 1 ), 1 )
                if( j > 0 && j % 2 !== 0 || j == 1 ) indices.push( i * this.res + j - 1, i * this.res + j )
            }
        }

        var offset = this.depth * this.res

        for( var i = 0 ; i < this.depth ; i++ ){
            for( var j = 0 ; j < this.res ; j++ ){
                vs.push( this.size - this.size / 2, -(i + 1) * this.step, j / ( this.res - 1 ) * this.size - this.size / 2 )
                uvs.push( 1, j / ( this.res - 1 ) )
                if( j > 0 && j % 2 !== 0 || j == 1 ) indices.push( offset + i * this.res + j - 1, offset + i * this.res + j )
                
            }
        }

        offset += this.depth * this.res
...
class Dots extends Points{
    constructor( size ){
        ...
        this.material = new ShaderMaterial({
            ...
            depthTest : false,
            // depthWrite : false,
            vertexShader : dotShader.vert,
            fragmentShader : dotShader.frag,
            transparent : true
        })
    }
}
...
class Water {
    constructor(){
        ...
        this.grid = new Grid( this.BOUNDS )
        this.grid.position.y += 150
        this.scene.add( this.grid )


        this.points = new Dots( this.BOUNDS )
    ...
    step( time ){
        ...
        this.points.material.uniforms.tex.value = this.waterMesh.gpuCompute.getCurrentRenderTarget( this.waterMesh.heightmapVariable ).texture
        if( this.grid ) this.grid.material.uniforms.tex.value = this.waterMesh.gpuCompute.getCurrentRenderTarget( this.waterMesh.heightmapVariable ).texture
