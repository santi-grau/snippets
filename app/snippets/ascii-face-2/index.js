...
class Snippet{
    ...
    process( inc ){
        ...
        for(var i = 0; i < this.data.length; i += 4 ) {
            ...
            var brightness = ( 0.34 * this.data[i] + 0.5 * this.data[i + 1] + 0.16 * this.data[i + 2] )
            
            if( Math.random() > inc ) curline.innerHTML += bogus[Math.floor(Math.random()*bogus.length)]
            else curline.innerHTML += chars[ Math.round( brightness / 255 * ( chars.length - 1 )  )  ]
